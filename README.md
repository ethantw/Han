...
2.  開啓`sass/han/ff.scss`，將「17、18行」的`$zhuyin_ttf`及`$zhuyin_eot`重新定向至正確的檔案路徑。
    ...
    **請注意：**此處使用的注音符號字體係「中華民國教育部」所研發的「[教育部標準楷書][kai]」，並**採[創用CC「姓名標示–禁止改作–3.0台灣版」][cc]授權**。若你有著作權的疑慮，請直接將上述提及的二行變數留空，「_漢_{.pn}字標準格式」將自動[[fallback]]{:en-GB}至使用者作業系統的_漢_{.pn}字手寫字體。
